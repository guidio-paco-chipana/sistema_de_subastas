...
public class Servlet_Usu extends HttpServlet {
    ...
    private void Login(HttpServletRequest request, HttpServletResponse response)
        ...
        //String op = request.getParameter("optionsRadios");
            
        try{
            //usuarios
            Usuario usu = Usuario_DB.VerificarUsuario(usuario);    
            /******************************************************************/
            if(usuario.equals(usu.getNombreUsuario())){
                if(clave.equals(usu.getClaveUsuario())){
                    System.out.println(usu.getTipoUsuario());                    
                   if("administrador".equals(usu.getTipoUsuario())||"ADMINSTRADOR".equals(usu.getTipoUsuario())){                      
                       //enviando parametro de session  
                        request.getSession().setAttribute("parametroCodigo", usu.getCodigoUsuario());
                        response.sendRedirect("Catalogo.jsp");
                        //response.sendRedirect("CuentaAdministrador.jsp");
                    }else{   
                       if ("cliente".equals(usu.getTipoUsuario())||"CLIENTE".equals(usu.getTipoUsuario())) {
                           //enviando parametro de session  
                        request.getSession().setAttribute("parametroCodigo", usu.getCodigoUsuario());
                        response.sendRedirect("Catalogo.jsp");
                        //response.sendRedirect("CuentaAdministrador.jsp");                                                   
                ...
                response.sendRedirect("respuesta.jsp?mens='Su nombre de usuario es incorrecto'"); 
            }                                        
    ...
    private void RegistrarUsuario(HttpServletRequest request, HttpServletResponse response)
        ...
        String Codigo_u = request.getParameter("txtCodigo_U");
        String Nombre = request.getParameter("txtNombres");
        String Nombre_U = request.getParameter("txtNombres_U");
        String Apellidos = request.getParameter("txtApellidos");
        String Email = request.getParameter("txtEmail");
        String Pais = request.getParameter("txtPais");
        String Direccion1 = request.getParameter("txtDireccion");
        String Clave = request.getParameter("txtClave");
        String Tipo_U = request.getParameter("txtTipoU");
        
        Usuario usu = new Usuario();
        
        usu.setCodigoUsuario(Codigo_u);
        usu.setNombre(Nombre);
        usu.setNombreUsuario(Nombre_U);
        usu.setApellidosUsuario(Apellidos);
        usu.setEmailUsuario(Email);
        usu.setPais(Pais);
        usu.setDireccion1Usuario(Direccion1);
        usu.setClaveUsuario(Clave);
        usu.setTipoUsuario(Tipo_U);
